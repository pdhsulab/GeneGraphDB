...
def _single(sample_id, google_bucket, distance, comment, outfile, samples_path="", clean_files=False):
    ...
    _load.load_proteins(sample_id, protein, samples_path)
    crisprid2crhash = crisprnode.load_CRISPRs(sample_id, samples_path)
    recid2contig = _load.load_fasta(sample_id, fasta, contigs, samples_path)
    _load.load_contig2sample(sample_id, contigs, samples_path)
    _load.load_coords(sample_id, sorted_gff_name, recid2contig, crisprid2crhash, samples_path)
    proteinnode.create_all_protein_crispr_edge_csv(sample_id, distance,
...
def bulk_connect_proteins_crisprs(max_distance, samples_path = ''):
    ...
    for sample_id in os.listdir(samples_path):
        try:
            ...
            os.chdir("../..")
            sample_id_path = sample_id + "/"
            proteinnode.create_all_protein_crispr_edge_csv(sample_id, max_distance, samples_path)
            # to do: debug this here and in other loadmulti.py file
            p2pcsv_paths.append(samples_path + sample_id_path + "protein2protein.tmp.csv")
            p2ccsv_paths.append(samples_path + sample_id_path + "protein2crispr.tmp.csv")
            p2p_window_csv_paths.append(samples_path + sample_id_path + "protein2protein_window.tmp.csv")
            p2c_window_csv_paths.append(samples_path + sample_id_path + "protein2crispr_window.tmp.csv")
    ...
    os_cmd_p2p += " > " + samples_path + "bigfile_p2p.csv"
    os_cmd_p2c += " > " + samples_path + "bigfile_p2c.csv"
    os_cmd_p2p_window += " > " + samples_path + "bigfile_p2p_window.csv"
    os_cmd_p2c_window += " > " + samples_path + "bigfile_p2c_window.csv"
    ...
    proteinnode.load_csv(samples_path + "bigfile_p2p.csv", samples_path + "bigfile_p2c.csv",
                            samples_path + "bigfile_p2p_window.csv", samples_path + "bigfile_p2c_window.csv")
    toc = time.time()
