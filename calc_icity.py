...
# final_icity_output(get_random10kprot(), list(cas2s_set))
...
tic = time.time()
neigh_temp = create_tnpB_neighbourhood(tnpBs_list)
toc=time.time()
...
def tnpB_icity_output():
    target_p100ids = []
    with open("bait_neighbourhood.txt", "r") as infile:
        lines = infile.readlines()
        for line in lines:
            neighb_id = line.split('\n')[0]
            target_p100ids.append(neighb_id)
    icity_arglist = [(target_p100id, target_p100ids) for target_p100id in target_p100ids] # to do - wastes hella memory
    icity_list = calc_icity_pool(icity_arglist)
    count_err = 0
